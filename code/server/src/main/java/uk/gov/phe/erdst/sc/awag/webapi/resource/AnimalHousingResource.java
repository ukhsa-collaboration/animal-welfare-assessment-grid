...
@Path(WebApi.RESOURCE_HOUSING_API)
public class AnimalHousingResource extends GlobalResource
{
    @Inject
    private AnimalHousingController mAnimalHousingController;

    /**
     * Create animal housing.
     * @param data
     ...
     */
    @POST
    public Response create(AnimalHousingClientData data, @Context SecurityContext sc)
        throws AWInputValidationException, AWNonUniqueException, AWSeriousException
    {
        return onCreatedSuccess(
            mAnimalHousingController.createAnimalHousing(data, WebSecurityUtils.getNewApiLoggedUser(sc)));
    }

    /**
     * Get all animal housings. Optionally response can be paged if <b>both</b> offset and
     ...
     * @param offset
     * @param limit
     * @return animal housings dto - includes paging information if offset and limit have been provided
     * @response mvn400ResponseJson
     */
        ...
        throws AWInputValidationException
    {
        return onRetrieveSuccess(
            mAnimalHousingController.getAllAnimalHousings(WebApiUtils.getPagingParams(offset, limit)));
    }

    /**
     * Get animal housing by id.
     * @param id
     *            has to be > 0
     * @return factor dto
     * @response mvn400ResponseJson
    ...
    {
        return onRetrieveSuccess(mAnimalHousingController.getAnimalHousing(id));
    }

    /**
     * Get animal housings filtered by name. The likeFilter query parameter is used to
     * match housing name property. Optionally response can be paged if <b>both</b> offset and
     * limit properties are specified.<br />
     * <i>offset</i> mvnPagingOffsetValidValueJavaDoc <br/>
     * <i>limit</i> mvnPagingLimitValidValueJavaDoc <br/>
     * @param likeFilter
    ...
    public Response getLike(@QueryParam(QUERY_PARAM_LIKE_FILTER) String likeFilter,
        ...
        throws AWInputValidationException
    {
        return onRetrieveSuccess(mAnimalHousingController.getHousingLike(WebApiUtils.getLikeFilterParam(likeFilter),
            WebApiUtils.getPagingParams(offset, limit)));
    }

    /**
     * Update details of a given animal housing.<br />
     * @param id
     *            has to be > 0
     * @param data
     * @return animal housing dto with updated details
     * @response mvn400ResponseJson
     * @response mvn404ResponseJson
     * @response mvn500ResponseJson
     */
    @Path(Constants.WebApi.PATH_ID_PARAM)
    @PUT
    public Response update(@PathParam(ID_PATH_PARAM) Long id, AnimalHousingClientData data, @Context SecurityContext sc)
        throws AWNonUniqueException, AWNoSuchEntityException, AWInputValidationException, AWSeriousException
    {
        return onUpdateSuccess(
            mAnimalHousingController.updateAnimalHousing(id, data, WebSecurityUtils.getNewApiLoggedUser(sc)));
    }

    /**
     * Upload a list of animal housings<br />
     * @param uploadFile
        ...
        throws AWInputValidationException, AWNonUniqueException
    {
        return onUploadSuccess(
            mAnimalHousingController.uploadAnimalHousing(uploadFile, WebSecurityUtils.getNewApiLoggedUser(sc)));
    }
