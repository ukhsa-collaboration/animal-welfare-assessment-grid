...
var templateMngmtFormEvents = {
    ...
    onTemplateUploadSelected: "onTemplateUploadSelected",
    onTemplateUploadClearSelected: "onTemplateUploadClearSelected"
};
...
manageTemplateControllers.controller('MngmtTemplateScaleCtrl', ['$scope', 'scaleService', 'formService',
    function ($scope, scaleService, formService) {
        ...
        $scope.$on(templateMngmtFormEvents.onTemplateUpdateForm, function (event, template) {
            formService.setSelect2Val([{
                id: template.templateScale.scaleId,
                text: template.templateScale.scaleName,
                locked: false
            }], jQuery('#' + that.selectId));
        });
    ...
    function ($scope, appConfig, templateService, assessmentService, $timeout) {
        ...
        this.addParameterAndFactors = function () {
            if (this.parameterAndFactorsToAdd.parameterId !== appConfig.config.unassignedId) {
                ...
                if (this.template.templateParameters) {
...
manageTemplateControllers.controller('MngmtTemplateUploadCtrl', ['$scope', 'appConfig', 'templateService', '$timeout',
    function ($scope, appConfig, templateService, $timeout) {
        ...
        this.onFileUploadChange = function (event) {
            ...
            $scope.$emit(templateMngmtFormEvents.onTemplateUploadSelected, event.target.files);
        };

        $scope.$on(templateMngmtFormEvents.onTemplateUploadClearSelected, function (event) {
...
manageTemplateControllers.controller('MngmtTemplateUploadManagementCtrl', ['$scope', 'appConfig', 'templateService', '$timeout',
    function ($scope, appConfig, templateService, $timeout) {
        this.fileList = {};
        ...
        var uploadTemplateCallback = function (data) {
            that.isSuccess = true;
            that.resetErrors();
        };
        ...
        this.uploadTemplate = function () {
            ...
            templateService.uploadTemplate(fd, uploadTemplateCallback, onErrorCallBack);
        };

        this.clearUploadTemplate = function () {
            this.fileList = {};
            this.setDisabled(true);
            this.resetErrors();
            $scope.$broadcast(templateMngmtFormEvents.onTemplateUploadClearSelected);
        };

        $scope.$on(templateMngmtFormEvents.onTemplateUploadSelected, function (event, fileList) {
